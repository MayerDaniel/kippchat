...
def get_last_message():
	...
	date = ''
	if LAST_READ == -1:
		c.execute("SELECT ROWID,text,is_from_me FROM message WHERE ROWID = (SELECT MAX(ROWID) FROM message)")
	else:
		c.execute("SELECT ROWID,text,is_from_me FROM message WHERE ROWID > " + str(LAST_READ))
